...
-- SODIKM
local inaccurateBadgeNotifications = {inaccuratebadgenotifications} -- filled by asset module

if inaccurateBadgeNotifications then
	game:GetService("BadgeService").BadgeAwarded:connect(function(message)
		-- user id parameter did not exist yet, so we need to parse the message and check if awardee is us
		awardeeName = message:gsub(" won .*$","") -- removes " won " and everything after it
		if localPlayer.Name == awardeeName then
