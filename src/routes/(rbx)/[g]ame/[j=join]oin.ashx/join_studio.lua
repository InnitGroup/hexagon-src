...
local closeConnection = game.Close:connect(function() 
	if 0 then
		...
		if true then pcall(function() game:HttpPost("https://www.roblox.com/auth/invalidate", "invalidate") end) end
	end
end)
...
local success, err = pcall(function()	
	...
	playerConnectSucces, player = pcall(function() return client:PlayerConnect({UserId}, "127.0.0.1", {ServerPort}, 0, threadSleepTime) end)
	if not playerConnectSucces then
		--Old player connection scheme
		player = game:GetService("Players"):CreateLocalPlayer({UserId})
		analytics("Created Player")
		client:Connect("127.0.0.1", {ServerPort}, 0, threadSleepTime)
	else
		analytics("Created Player")
	end
	...
	if true then
		...
		delay(300, function()
			while true do
				pcall(function() game:HttpPost("https://www.roblox.com/auth/renew", "renew") end)
				wait(300)
			end
		end)
	end
